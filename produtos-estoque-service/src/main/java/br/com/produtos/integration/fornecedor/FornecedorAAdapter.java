...
public class FornecedorAAdapter implements SupplierPort {
    ...
    public BulkOrderResponse sendBulkOrder(BulkOrderRequest bulkOrder) {
        ...
        log.info("[FORNECEDOR-A] Enviando pedido em lote com {} itens para {}",
        ...
        try {
            ...
            log.info("[FORNECEDOR-A] Resposta em lote recebida: {} - {}",
                     response.getBody().getProtocol(), response.getBody().getStatus());

            return response.getBody();
        } catch (Exception e) {
            log.error("[FORNECEDOR-A] Erro ao comunicar com fornecedor (bulk): {}", e.getMessage());
